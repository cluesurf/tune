...
- Fricative pairs across vowels in CVC sequences: same pair always blocked (`s-s`, `s-z`, `z-s`, `z-z`, and likewise `f-f`/`f-v`/`v-f`/`v-v`, `c-c`/`c-C`/`C-c`/`C-C`, `j-j`/`j-x`/`x-j`/`x-x`). Cross-pair allowed only if voicing matches (e.g. `f-s` OK, `f-C` blocked)
- Max 1 `x` per word
- Max 1 `c`/`C` per word
