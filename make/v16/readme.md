...
```
every-*.txt     every form the rules ALLOW        170,406   v16:every
ceiling-*.txt   the most that fit at distance 2    52,795   v16:ceiling
...
Everything else in `base/v16/` is working output from an earlier pass:
`cvc.txt` and friends, `ratio-*.txt`, `short-apart.txt`. **Check the
write time before reading one.** They are regenerated by `v16:pick` and
`v16:ratio` and do not refresh when `v16:final` runs, so a rule change
leaves them silently wrong.
