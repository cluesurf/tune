/**
 * Word generation for Tune Tree.
 *
 * Tree has 9 sounds and one syllable shape, CV. A root is one, two or
 ...
 *
 * Tree is small enough that the whole legal space is kept. Rock has to
 * select and Moon has to be hand tuned, but Tree does not: the rules
 * in `sound.ts` are exact, so the lexicon is everything that survives
 * them.
 *
 * Usage:
...
rule('CORRESPONDENCE WITH TUNE ROCK')
const check = checkCorrespondence()
if (check.ok) {
  line('every Rock sound has exactly one Tree ancestor')
  line('3 vowels and 14 consonants accounted for, and the breath was lost')
} else {
