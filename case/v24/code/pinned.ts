...
import { TWIN_W, everyRoot, templateOf, type Root } from './rule'

const here = dirname(fileURLToPath(import.meta.url))
const BASE = resolve(here, '../base/term')
...
function why(form: string) {
  ...
  // `w` is a root letter in v24 unless the `w` seam is put back, so the
  // reason has to ask rather than assert. Saying "v24 reserves w" while
  // v24 spells `wot` would be a guide lying about its own language.
  if (TWIN_W && form.startsWith('w')) {
    return 'opens on w, which the w seam reserves'
  }
  if (form.length === 5) return 'two syllables, which v24 has no shape for'
