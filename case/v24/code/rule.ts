...
/**
 * THE `w` FORM, WHICH v24 DROPPED AND CAN PUT BACK.
 *
 * v17 wrote a doubled sound once and followed it with a `w`, so
 * `bas + sag` was `baswag`. v24 writes the sound once and nothing
 * else, `basag`. The reader knows a root is missing its first sound
 * because a VOWEL stands where an onset should, and which sound it
 * was is fixed: a twin is two of the same consonant.
 *
 * **That one change frees the letter `w` entirely**, because nothing
 * else in the language ever wrote one. So `w` opens roots now, which
 * is 17 v16 pins back, and `kw sw tw` join the onsets.
 *
 * Set `V24_TWIN_W=1` to write `baswag` again and take the `w` back
 * out of the alphabet. Everything downstream reads this one flag.
 */
export const TWIN_W = process.env.V24_TWIN_W === '1'

export const VOWEL = 'i e a o u'.split(' ')

/** Set `V17_DIPHTHONG=0` to weigh the language without them. */
export const DIPHTHONG =
  process.env.V17_DIPHTHONG === '0' ? [] : 'ai au oi'.split(' ')

/**
 * THE ONE PAIR OF NUCLEI THAT COUNT AS NEAR.
 *
 * Two different nuclei are otherwise always far enough apart on their
 * own, which is what keeps `bat` and `bot` both standing. `ai` and
 * `oi` are the exception: they close on the same glide and the vowels
 * before it are near, so `kais` against `kois` is one soft cue and the
 * distance rule has to see it.
 *
 * `au` is near neither, because it closes on a different glide.
 */
export const NEAR_NUCLEUS = new Map<string, Array<string>>([
  ['ai', ['oi']],
  ['oi', ['ai']],
])
...
/**
 * `w` OPENS A ROOT UNLESS THE `w` SEAM IS PUT BACK.
 *
 * It was reserved in v17 for one job, marking a dropped twin, and a
 * letter held back for one job that no longer exists is a letter
 * thrown away. 17 of the 25 v16 pins v17 could not spell open on it:
 * `water`, `with`, `we`, `what`, `why`, `walk`, `wait`, `wave`.
 */
export const ONSET_ONE = (
  TWIN_W ? 'mnbdgptkhszfvxjcCylr' : 'mnbdgptkhszfvxjcCylrw'
).split('')
export const CODA_ONE = 'mnqbdgptkszfvxjcClr'.split('')

/**
 * THE LEVERS, so the pool can be weighed without a second copy of this
 * file.
 *
 * Each is a MEASUREMENT switch and not a setting: the language is what
 * this file says with none of them set. They exist because the only
 * honest answer to "can we have more roots" is a number per lever, and
 * getting one used to mean editing the lists and remembering to put
 * them back.
 *
 * ```text
 * V17_DIPHTHONG=0     weigh it with no ai and au
 * V17_ADD_ONSET='sf'  add onsets, space separated
 * V17_ADD_CODA='ps'   add codas
 * V17_NO_TIGHT=1      drop the four pair rule inside clustered roots
 * ```
 */
...
export const ONSET_TWO = [
  ...
  // `kw sw tw` need the `w` seam gone, or a joiner would sit inside a
  // root and the reader could not tell the two apart.
  ...(TWIN_W ? [] : 'kw sw tw'.split(' ')),
  ...more('V17_ADD_ONSET'),
]
...
export const CODA_TWO = [
  ...(
    ...
    'ndj ntx qkc ' +
    // The voiced halves of pairs the list already had, plus the two
    // liquid-nasal codas. `lv` answers `lf`, `gd` answers `kt`.
    'gd kt rm rn lv'
  ).split(' '),
  ...more('V17_ADD_CODA'),
]
...
for (const [a, b] of process.env.V17_NO_TIGHT === '1'
  ? []
  : [
      ['p', 'b'],
      ['t', 'd'],
...
export function nearGraph(group: Array<Root>) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    // And the NUCLEUS, which the walk above skips because two vowels
    // are normally far enough apart on their own. `ai` against `oi` is
    // the one pair that is not.
    for (const s of NEAR_NUCLEUS.get(root.nuc) ?? []) {
      tie(i, at.get(root.on + s + root.co))
    }
  }
  return edge
}
...
export function seam(a: Root, b: Root, doubt = false): Seam {
  ...
  switch (kind) {
     ...
     */
    /**
     * A DOUBLED SOUND IS SAID ONCE, and nothing marks the seam.
     *
     * `bas + sag` is `basag`. The reader finds the seam because a
     * VOWEL stands where an onset should, and no root opens on one.
     * Under `V24_TWIN_W` it is `baswag` instead.
     *
     * Against a cluster there is no clean single sound to drop, so
     * both roots stay whole and a liquid goes between, as before.
     */
    case 'twin':
      return cluster
        ? liquid(a, b)
...
export const spell = (one: Root, dropped: boolean, mark: string) =>
  (dropped ? `${one.on.slice(1)}${TWIN_W ? 'w' : ''}` : one.on) +
  one.nuc +
  mark +
  one.co
...
export function read(
  ...
  const walk = (at: number, prev: Root | null, pending: string) => {
    ...
    /**
     * A VOWEL HERE, so the root lost its first sound to a twin.
     *
     * Which sound it was is not a guess: a twin is two of the SAME
     * consonant, so the missing one is whatever the left root closes
     * on. `bas + sag` is `basag`, and standing at the `a` the reader
     * asks only for roots opening on `s`.
     *
     * Under `V24_TWIN_W` the same seam writes `baswag` instead, and
     * then it is the `w` that marks the spot rather than the vowel.
     */
    if (!TWIN_W && VOWEL.includes(text[at])) {
